...
import {serializeNonPOJOs} from "$lib/utils/pojo";
...
export const authentication: Handle = async ({event, resolve}) => {
    ...
    if (event.locals.pb.authStore.isValid) {
        // if the auth store is valid, set the 'pb.auth' model to the store's model
        event.locals.user = serializeNonPOJOs(event.locals.pb.authStore.model);
    } else {
        // if the auth store is invalid, clear the 'pb.auth' model
